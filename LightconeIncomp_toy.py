...
def toyincomp(toy_number):
    # Import the lightcone data
    Lightcone1e91e10 = np.load(DATA_DIRECTORY + '{}_Lightcone_lumcut1e91e10.npy'.format(unique_id))
    Lightcone1e101e11 = np.load(DATA_DIRECTORY + '{}_Lightcone_lumcut1e101e11.npy'.format(unique_id))
    Lightcone1e111e12 = np.load(DATA_DIRECTORY + '{}_Lightcone_lumcut1e111e12.npy'.format(unique_id))
    ...
    incomp_func1[:] = incomp_func1[:] * invert_gaussian2(new_z_axis, 5069, 20, 0.7, 6000, 50, 0.5)
    ...
    np.save(DATA_DIRECTORY + '{}_Lightcone_lumcut1e91e10_{}inc.npy'.format(unique_id, toy_number), Lightcone1e91e10_inc)
    np.save(DATA_DIRECTORY + '{}_Lightcone_lumcut1e101e11_{}inc.npy'.format(unique_id, toy_number), Lightcone1e101e11_inc)
    np.save(DATA_DIRECTORY + '{}_Lightcone_lumcut1e111e12_{}inc.npy'.format(unique_id, toy_number), Lightcone1e111e12_inc)
    ...
    plt.savefig(PLOT_DIRECTORY + '{}_CompletenessVsRedshift_{}.pdf'.format(unique_id, toy_number), bbox_inches='tight')
    ...
    # Save the plot as a pdf file.
    plt.savefig(PLOT_DIRECTORY + '{}_CompletenessVsRedshift_bolo_{}.pdf'.format(unique_id, toy_number), bbox_inches='tight')
    # plt.show()
    ...
    for i in range(7):
        ...
        df.to_csv(DATA_DIRECTORY + '{}_MBII_{}inc_lc_corrfunc_z{}.csv'.format(unique_id, toy_number, redshifts[i]), index=False)
        ...
        plt.grid(visible=False)
        plt.gca().set_box_aspect(1)
        plt.savefig(PLOT_DIRECTORY + '{}_MBII_{}inc_lc_zhist_z{}.pdf'.format(unique_id, toy_number, redshifts[i]), bbox_inches='tight')
        # plt.show()
    ...
    # plt.savefig(PLOT_DIRECTORY + '{}_MBII_toy1inc_lc_corrfunc.pdf'.format(unique_id))
